...
// Functional component TopBar
export const TopBar = () => {
    // Extracting isAuth and signInWithGoogle from AuthContext
    ...
    return (
        <div className="w-full bg-white"> {/* Main container with full width and white background */}
            <div className="flex flex-wrap items-center justify-between w-full px-5 py-3"> {/* Flex container for content alignment */}
                {/* Logo and home link */}
                <Link href="/">
                ...
                <div
                    ...
                    {/* Pricing link, shown only if user is not premium */}
                    {!isPremium && (
                        <Link href="/pricing">
                    ...
                    {isAuth ? (
                        ...
                        >
                            <Link href="/account">
                                <p className="text-xl text-black">My Account</p> {/* Account link */}
                            </Link>
                        </button>
                    ) : (
                        <button
                            ...
                            className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-500 rounded-full shadow transition duration-200 hover:shadow-md focus:shadow-md"
                        >
                            <svg className="h-5 w-5" version="1.1" viewBox="0 0 48 48"> {/* Google icon */}
                                <path fill="#EA4335"
                            ...
                            <span className="text-black font-medium">Sign in with Google</span> {/* Sign-in button text */}
                        </button>
                    )}
                </div>
            </div>
